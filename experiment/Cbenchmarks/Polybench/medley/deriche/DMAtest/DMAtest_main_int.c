		 ...
		 uint32_t imgOut[W][H])
{
  ...
  for (i = 0; i < w; i++)
     for (j = 0; j < h; j++){
	      // imgIn[i][j] = (int) ((313*i+991*j)%65536) / 65535.0f;
        imgIn[i][j] = i * h + j;
     }
...
void print_image1and2(int w, int h,
		 /*DATA_TYPE POLYBENCH_2D(imgOut,W,H,w,h)*/uint32_t imgOut1[W][H], uint32_t imgOut2[W][H])
