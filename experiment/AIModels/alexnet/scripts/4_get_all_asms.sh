...
srcfolder2="$rootfolder/$IRfolder/DontOffloadKernels"
...
############
## Cann't offload kernels
############
for file in "$srcfolder2"/*.mlir; do
    ...
    if [[ -f "$file" ]]; then
      ...
      $file -o $rootfolder/$IRfolder/tempfiles/"3_${filename}_llvm.mlir" 

      mlir-translate  --mlir-to-llvmir \
        $rootfolder/$IRfolder/tempfiles/"3_${filename}_llvm.mlir" \
        -o "$rootfolder/$IRfolder/tempfiles/$filename.ll"
      
      opt -O3  "$rootfolder/$IRfolder/tempfiles/$filename.ll" \
          --disable-builtin=memset  --mtriple=riscv64 --mcpu=rocket-rv64  \
          -o "$rootfolder/$IRfolder/tempfiles/${filename}_opt.bc"

      llc -O3 "$rootfolder/$IRfolder/tempfiles/${filename}_opt.bc" \
...
mlir-opt \
 ...
 --cse --canonicalize \
 --reconcile-unrealized-casts \
 $rootfolder/$IRfolder/02-affine_host.mlir -o $rootfolder/$IRfolder/"3_${func_name}_llvm.mlir" \
